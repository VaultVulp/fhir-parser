...
tpl_resource_target_ptrn = './fhirmodels/{}.py'            # where to write the generated class files to, with one placeholder for the class name

# Whether and where to put the factory methods
write_factory = True
tpl_factory_target = './fhirmodels/fhirelementfactory.py'

# Whether and where to write unit tests
write_unittests = True
tpl_unittest_target_ptrn = './fhirmodels/tests/test_{}.py'     # a pattern to determine the output files for unit tests; the one placeholder will be the class name
...
manual_profiles = [                                     # all these profiles should be copied to dirname(`tpl_resource_target_ptrn`): tuples of (path, module, profile-name-list)
    ...
    ('Python/fhirdate.py', 'fhirdate', ['date', 'dateTime', 'instant']),
